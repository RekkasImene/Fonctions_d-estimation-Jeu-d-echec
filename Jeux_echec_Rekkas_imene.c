...
/*************************** Une fonction d'estimation proposée par Rekkas Imene ***********************************/


/*Fonction ajoutéé*/
int ControleCentre (struct config *conf, int mode) ///** on attribue un bonus à celui qui domine le centre
{
    int coefAdv = 3;
    int ctrlCJ=0, ctrlCA=0; /// ctrlCJ : control zone centre joueur ///  ctrlCA : control zone centre adversaire

    if (mode == MAX) /// on va vérifier les cases du milieux qui sont menasées par le joueur Maximisant (l'utilisateur)
    {
        if (caseMenaceePar(MAX,3,3,conf)) ctrlCJ++; /// la case d4 est menacée
        if (caseMenaceePar(MAX,3,4,conf)) ctrlCJ++; /// la case e4 est menacée
        if (caseMenaceePar(MAX,4,3,conf)) ctrlCA++; /// la case d5 est menacée
        if (caseMenaceePar(MAX,4,4,conf)) ctrlCA++; /// la case e5 est menacée
    }
    else  // MIN (la machine)
    {
        if (caseMenaceePar(MIN,3,3,conf)) ctrlCA++;
        if (caseMenaceePar(MIN,3,4,conf)) ctrlCA++;
        if (caseMenaceePar(MIN,4,3,conf)) ctrlCJ++;
        if (caseMenaceePar(MIN,4,4,conf)) ctrlCJ++;
    }

    return (ctrlCJ + coefAdv*ctrlCA);
}




int FormationPions(struct config *conf, int mode)
{
    int i,j, score=0;
		...
		if (conf->mat[i][j] == 'p')
		 {
		     if (i>0)
             {
                 if (j>0) if (conf->mat[i-1][j-1] == 'p') score++;
		         if (j<7) if (conf->mat[i-1][j+1] == 'p') score++;
             }
		     if (i<7)
             {
                 if (j>0) if (conf->mat[i+1][j-1] > 0) score++;
                 if (j<7) if (conf->mat[i+1][j+1] > 0) score++;
		         if (conf->mat[i+1][j] == -'p') score--;
		         if (conf->mat[i+1][j] == 'p') score--;
             }

		     if (j<7) if (conf->mat[i][j+1] == 'p') score--;
		 }
	   }
		...
		if (conf->mat[i][j] == -'p')
		 {
		     if (j>0)
             {
                if (conf->mat[i+1][j-1] == -'p') score++;
             }
		     if (i>0)
             {
                 if (conf->mat[i-1][j+1] < 0) score++;
                 if (conf->mat[i-1][j] == 'p') score--;
		         if (conf->mat[i-1][j] == -'p') score--;
             }
             if (j<7 && i<7)
...
int FouDevantPionsCentraux(struct config *conf, int mode)
{
    int i,j,score=0;
    if (mode == MAX)
    {
       if (conf->mat[0][5] == 0)
       {
           if (conf->mat[2][3] == 'p' && conf->mat[3][4] == 'p') score--;
       }
       if (conf->mat[0][2] == 0)
       {
           if (conf->mat[2][4] == 'p' && conf->mat[3][3] == 'p') score--;
       }
    }
    else
    {
        if (conf->mat[7][5] == 0)
       {
           if (conf->mat[5][3] == 'p' && conf->mat[4][4] == 'p') score--;
       }
       if (conf->mat[7][2] == 0)
       {
           if (conf->mat[5][4] == 'p' && conf->mat[4][3] == 'p') score--;
       }
    }
...
int DistanceRoque (struct config *conf, int mode) ///* on attribue un bonus à celui qui est proche de (ou bien peut faire) le roque
{ int coefGrRoque = 2;
  int n=0, m=0, score=0;
  struct config T[100];
    if (mode == MAX)
    {
        ...
        {
            deplacementsB(conf,0,5,T,&n);
            deplacementsB(conf,0,2,T,&m);
            if (conf->mat[0][5] == 0 || n != 0) /// la case du fou droite est vide ou bien il peut se déplacer
            {
                score++;
            }
            if (conf->mat[0][6] == 0) /// la case du cavalier droite est vide
            {
                score ++;
            }
            if (conf->mat[0][2] == 0 || m != 0) /// la case du fou gauche est vide ou bien il peut se déplacer
            {
                score= score + coefGrRoque;
            }
            if (conf->mat[0][1] == 0) /// la case du cavalier droite est vide
            {
                score= score + coefGrRoque;
            }
        }
    ...
    else
    {
        ...
        {
            deplacementsN(conf,7,5,T,&n);
            deplacementsN(conf,7,2,T,&m);
            if (conf->mat[7][5] == 0 || n != 0) /// la case du fou droite est vide ou bien il peut se déplacer
            {
                score++;
            }
            if (conf->mat[7][6] == 0) /// la case du cavalier droite est vide
            {
                score ++;
            }
            if (conf->mat[7][2] == 0 || m != 0) /// la case du fou gauche est vide ou bien il peut se déplacer
            {
                score = score+coefGrRoque;
            }
            if (conf->mat[7][1] == 0) /// la case du cavalier droite est vide
            {
                score = score+coefGrRoque;
            }
        }
    ...
    return score;
}/*end of fonction ajoutée*/
/*fonction d'estimation proposée*/
int estim7( struct config *conf )
{
	int i, j,n, m ;
	int SCORE=0, EvalMateriel=0, degreeliberte=0, ctrlCntr=0, distRoque=0, formaPions=0, FDPC=0;
	int coefDl=2, coefCtrlCntr=5, coefRoque=4, coefFormPion = 3, coefFDPC = 4;
	int pionB = 0, pionN = 0, cfB = 0, cfN = 0, tB = 0, tN = 0, nB = 0, nN = 0; /// le cavalier et le fou auront le mm coef cfB
    struct config T[100];
	...
	// Somme pondérée de pièces de chaque joueur.
	EvalMateriel = ( (pionB + cfB*6 + tB*8 + nB*20) - (pionN + cfN*6 + tN*8 + nN*20) ); /// Evaluation du matériel ça donne
                                                                                              /// au maximum 56

    /// ********************************* calcul du degree de libertee ******************************** ///
   // generer_succ(conf,MAX,T, &n); /// n contient le nombre de tt les coups possible pour le Blanc(Max)
    //printf("generer_succ(conf,MAX,T, &n);blanc ");
    //generer_succ(conf,MIN,T, &m); /// m contient le nombre de tt les coups possible pour le Noir(Min)a
    //printf("generer_succ(conf,MAX,T, &n); noir ");
    //degreeliberte = n - m; /// ça donne au maxilmum 15
    /// *********************************************************************************************** ///

    /// ********************************* calcul du Controle du centre ******************************** ///
    ctrlCntr = ControleCentre(conf,MAX) - ControleCentre(conf,MIN); /// ça donne au maximum [-8, 8]
    /// *********************************************************************************************** ///

    /// ********************************* calcul du Distance du roque ******************************** ///
    distRoque = DistanceRoque(conf,MAX) - DistanceRoque(conf,MIN); /// ça donne au maximum [-6, 6]
    /// *********************************************************************************************** ///

    /// ********************************* calcul de la formation des pions **************************** ///
    formaPions = FormationPions(conf,MAX) - FormationPions(conf,MIN); /// ça donne au maximum [-14, 14]
    /// *********************************************************************************************** ///

    /// ********************************* Fou devant Pions centraux **************************** ///
    FDPC = FouDevantPionsCentraux(conf,MAX) - FouDevantPionsCentraux(conf,MIN); /// ça donne au maximum [-2,2]
    /// *********************************************************************************************** ///
// Le facteur 0,54 pour ne pas sortir de l'intervalle ]-100 , +100[
    SCORE = (EvalMateriel + coefCtrlCntr*ctrlCntr + coefRoque*distRoque + coefFormPion*formaPions + coefFDPC*FDPC)*0.54;
    //max 170*0,54 = 95

	/*if (EvalMateriel > 95) EvalMateriel = 95;		// pour rétrécir l'intervalle à
	if (EvalMateriel < -95) EvalMateriel = -95;
	//printf("%d",EvalMateriel);	// ]-95 , +95[ car ce n'est qu'une estimation
    return EvalMateriel; */
